...
def test_live_transport_tools_tokens_evidence_and_reproducible_config(tmp_path):
    with model_server() as (port, calls), system(tmp_path) as c:
        ...
        assert [x[0] for x in calls] == ["/api/tags", "/api/generate"] * 3
        completions = [payload for path, payload in calls if path == "/api/generate"]
        assert all(p["options"]["num_predict"] == 256 and p["stream"] is False for p in completions)
        assert all(p["options"]["num_ctx"] == 2304 and p["think"] is False for p in completions)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"models": []},
        {"models": "bad"},
        {"models": [{"name": "LiquidAI/lfm2.5-1.2b-instruct:latest", "digest": "0" * 64}]},
    ],
)
def test_inventory_fails_before_generation(tmp_path, inventory):
    with model_server(lambda path, reply: inventory) as (port, calls), system(tmp_path) as c:
        scenario, script = inputs(port)
        trial = c.run(scenario, local_model_policy(), script)
...
@pytest.mark.parametrize(
    "field,value",
    [("remote_host", "https://example.com"), ("remote_model", "remote"), ("digest", "0" * 64)],
)
def test_remote_or_changed_model_never_generates(tmp_path, field, value):
    def alter(path, reply):
        if path == "/api/tags":
            reply["models"][0][field] = value
        return reply

    with model_server(alter) as (port, calls), system(tmp_path) as c:
        scenario, script = inputs(port)
        trial = c.run(scenario, local_model_policy(), script)
        assert c.store.summary(trial)["outcome"] == "error"
        assert len(calls) == 1
...
@pytest.mark.parametrize("phase", ["/api/tags", "/api/generate"])
...
def test_input_admission_rejects_large_prompt_before_io(tmp_path):
    with model_server() as (port, calls), system(tmp_path) as c:
        scenario, script = inputs(port)
        script = script.model_copy(update={"max_input_tokens": 64})
        trial = c.run(scenario, local_model_policy(), script)
        assert c.store.summary(trial)["outcome"] == "error"
        assert calls == []
    ...
    async def check():
        ...
        async with server:
            ...
            with pytest.raises(TimeoutError):
                await _supervised_post(port, "/api/tags", {}, 0.15, lambda: None)
    ...
    async def check():
        ...
        async with server:
            ...
            with pytest.raises(
                ...
                await _supervised_post(port, "/api/tags", {}, 1, lambda: None)
